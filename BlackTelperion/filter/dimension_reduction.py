...
def PCA(blackdata, bands=20, band_range=None, step=5, mask : np.ndarray = None):
    """
    Apply a PCA dimensionality reduction to the hyperspectral dataset using singular vector decomposition (SVD).

    Args:
        data: the dataset (BlackData object) to apply PCA to.
    ...
    wav = None
    decomp = False
    if isinstance(blackdata, BlackData):
        wav = blackdata.get_wavelengths()

        if blackdata.is_int():
            blackdata.decompress()  # MNF doesn't work very well with ints....
            decomp = True  # so we can compress again afterwards

        data = blackdata.data.copy()
    else:
        data = blackdata.copy()
    ...
    if band_range is None:  # default to all bands
        ...
        if isinstance(band_range[0], int) and isinstance(band_range[1], int):
            ...
            assert isinstance(blackdata, BlackData), "Error - no wavelength information found."
            minb = blackdata.get_band_index(band_range[0])
            maxb = blackdata.get_band_index(band_range[1])
    ...
    if isinstance(blackdata, BlackData):
        outobj = blackdata.copy(data=False)
        outobj.data = out[..., 0:bands]
    else:
        outobj = BlackData( out[..., 0:bands] )
...
def MNF(blackdata, bands=20, band_range=None, noise='diff', noise_thresh=50, denoise=False, mask : np.ndarray =None):
    """
    Apply a minimum noise filter to a hyperspectral image.

    Args:
        blackdata: A BlackData instance containing the source dataset (e.g. image or point cloud).
    ...
    if (noise is None) and 'band noise' in blackdata.header:  # get noise from header?
        assert False, "Error - this is not yet implemented! [ hopefully soon! ]"
        noise = np.array( blackdata.header.get_list('band noise') )
    if (noise is not None) and (noise != 'diff'): # we have noise info - use it
        assert False, "Error - this is not yet implemented! [ hopefully soon! ]"
        assert noise.shape[0] == blackdata.band_count(), "Error - noise provided for %d bands but data has %d" % (noise.shape[0], blackdata.band_count())
    ...
    # denoise and export
    if denoise:
        out = blackdata.copy(data=False)
        out.data = mnfr.denoise(data, num=bands)
        out.set_wavelengths(blackdata.get_wavelengths()[valid_bands])
    else:
        out = blackdata.copy(data=False)
        out.header.drop_all_bands()  # drop band specific attributes
        out.data = reduced.reshape(blackdata.data.shape[:-1] + (bands,))
        out.push_to_header()
...
def from_loadings(data, L, m=None):
    ...
    Args:
       data: A dataset (BlackData instance or numpy array) with b bands in the last axis.
    ...
    Returns:
       a blackdata instance or numpy array containing the transformed data.
    """
    # get relevant data
    ...
    # return BlackData or numpy array
    if isinstance(data, BlackData):
