...
def TPT(data, sigma=10., window=21, n=2, thresh=0, domain=None, weighted=True, maxima=True, minima=True, vb=True):
    ...
    Args:
        data: a BlackData instance containing the spectra to filter.
...
def TPT2MWL(pos, depth, wmin=0, wmax=-1, data=None, vb=True):
    ...
    Args:
        ...
        data: a BlackData instance to use as a template for outputs. If not None (default) then this will return a BlackData instance.
        vb: True if a progress bar should be created. Default is True.

    Returns:
        a numpy array or BlackData instance (if data is not none), containing 4 bands (pos, width, depth, strength).
    """
    ...
    if data is None:  # return a numpy array directly
        return out
    else:  # make a copy of data and return BlackData instance
