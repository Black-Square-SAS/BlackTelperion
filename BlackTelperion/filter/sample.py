...
from BlackTelperion import BlackData, BlackLibrary
import numpy as np
...
def getClassLibrary( data, classification, skip0=True ):
    ...
    assert isinstance(data, BlackData), "Error - data must be a BlackData instance not %s" % type(data)
    assert isinstance(classification, BlackData), "Error - classification must be a BlackData instance not %s" % type(classification)
    ...
    return BlackLibrary(n, np.vstack(refl),
...
class Resample( object ):
    ...
    def apply(self, data):
        """
        Apply this resampling to a BlackData instance and return a new instance with appropriately averaged bands.

        Args:
            data: the BlackData instance to apply this sampling scheme to.
        Returns: A copy of the original BlackData instance with the bands averaged as defined. Corresponding wavelengths will be set to the middle of each averaged region.
        """
