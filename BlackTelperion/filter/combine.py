...
import numbers
import numpy as np
from BlackTelperion import io as io
...
def combine(image_list, method="median", warp=False):
    ...
    if warp:
        ...
        bnd1 = io.BlackImage.to_grey(arr[0][:, :, match_idx])  # we use the middle band to do alignment

        for i, image in enumerate(arr[1:]):
            print("Warping image %d" % (i + 1))

            # extract matching band
            bnd2 = io.BlackImage.to_grey(image[:, :, match_idx])
