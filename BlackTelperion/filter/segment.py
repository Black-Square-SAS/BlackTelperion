...
def readAnnotations(annot, hsi, mask_patches=False, mask_panels=False, mask_points=False, mode='average', inplace=False):
    ...
    # check inputs
    assert isinstance(annot, BlackTelperion.BlackImage), 'Error - annot (%s) is not a BlackImage instance' % type(annot)
    assert isinstance(hsi, BlackTelperion.BlackImage), 'Error - hsi (%s) is not a BlackImage instance' % type(hsi)
...
def label_blocks(image, fg=None, s=8, epad=20, boost=3, erode=3, bands=BlackTelperion.RGB, vb=True, **kwds):
    """
    ...
    Args:
        ...
        bands: the bands to use for segmentation. Default is BlackTelperion.RGB.
        vb: True if figures should be generated for QAQC. Default is True.
        **kwds: keywords are passed to BlackImage.quick_plot( ... ) if vb is true.
    """
    ...
    cls = BlackTelperion.BlackImage(labels[:, :, None])
...
def extract_tiles(image, labels, connected=False, ids=None, erode=0):
    ...
    Args:
        ...
        labels: a BlackImage instance or numpy array with different samples labelled as non-0 values.
    ...
    # get labels array
    if isinstance(labels, BlackTelperion.BlackImage):
    ...
    for i in ids:  # loop through labels
        ...
        if not connected:
            ...
            tile = BlackTelperion.BlackImage(np.full((np.max(local[:, 0] + 1),
...
def group_tiles( tiles, groups, ids=None, rotate=True, ignore=[] ):
    ...
    Args:
        tiles: a list of BlackImage tiles.
    ...
    # make output images
    tiled_image = BlackTelperion.BlackImage(data, header = tiles[0].header.copy())

    tiled_label = BlackTelperion.BlackImage(label)
    ...
    if ids is not None:
        tiled_id = BlackTelperion.BlackImage(sid)
...
def build_core_template(images, N=5, thresh=40, vb=True):
    ...
    # return
    return BlackTelperion.BlackImage(labels_im.T)
...
def tray_to_stick(tray, N):
    ...
    Args:
        tray: a BlackImage instance containing a linearly unwrapped tray.
    ...
    images = []
    for i in range(N):
        image = BlackTelperion.BlackImage(slices[i], header=tray.header.copy())
...
def composite_cores(trays, pad=2):
    ...
    Args:
        trays: a list of BlackImage instances containing unwrapped core trays. The header of these
    ...
    # create output image
    out = BlackTelperion.BlackImage(data)
...
def plot_drillhole(composite, N=5, maxN=25, **kwds):
    ...
    *Keywords*: are passed to BlackImage.quick_plot( ... )
    """
    ...
    for i in range(nrows):
        ...
        # make new BlackImage and store
        img = BlackTelperion.BlackImage(data)
