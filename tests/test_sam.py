...
class MyTestCase(unittest.TestCase):
    def test_sam(self):
        from BlackTelperion.analyse.sam import SAM, spectral_angles
        ...
        for data in  [image]:#[image, cloud]:
            ...
            lib = BlackTelperion.BlackLibrary( arr, lab=['EM1','EM2'], wav=data.get_wavelengths() )
            sam = SAM(data, lib)
            self.assertTrue(np.isfinite(sam.data).any())
            self.assertEqual(int(sam.X(onlyFinite=True)[-1, 0]), 1 )
            self.assertEqual(int(sam.X(onlyFinite=True)[-10, 0]), 0 )